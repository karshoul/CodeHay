...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if ($result_check->num_rows > 0) {
        header("Location: register.php?message=Tên đăng nhập hoặc Email đã tồn tại.&type=error"); // Đã sửa thành register.php
        exit();
    }
    $stmt_check->close();
    ...
    if ($stmt_insert->execute()) {
        ...
        header("Location: register.php?message=Có lỗi xảy ra khi đăng ký. Vui lòng thử lại.&type=error"); // Đã sửa thành register.php
        exit();
    }
    ...
    header("Location: register.php"); // Đã sửa thành register.php
    exit();
}
?>